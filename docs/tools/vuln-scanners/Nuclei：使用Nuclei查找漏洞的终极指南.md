...
假设你看到一篇关于“x-debug”HTTP请求头的有趣研究文章，这让你开始思考互联网上的应用程序和框架中是否潜藏着一些类似的有趣且未记录的头部。
...
这次，我们只是使用了基本的HTTP GET请求，而不是raw模式。我们为请求数据指定了几个属性，包括HTTP方法、路径和一些请求头。在这种情况下，我们在“headers”字段中添加了我们的“x-php-debug”头部。
...
为了演示Nuclei的一些高级的模板特性，我们将逐步介绍一个假设的Web应用程序，该应用程序存在远程代码执行漏洞，但需要有效的认证会话和CSRF令牌才能随请求发送。exploit-tools需要多个步骤。
...
5. 提交一个请求以创建数据库备份，其中包括exploit-tools有效载荷和CSRF令牌（POST /backup_db.php）
...
网络模板可以选择性地指定一个TCP端口，期望服务监听在这个端口上如上例所示，我们尝试连接到6101-6103端口。如果我们在模板中制定了端口，则需要使用&lbrace;&lbrace;host&rbrace;&rbrace;。如果在模板中不硬编码TCP端口号，则需要使用&lbrace;&lbrace;Hostname&rbrace;&rbrace;，并且用户需要在命令行或目标列表文件中提供端口作为目标主机规范的一部分。例如：
```yaml
...
与HTTP模式一样，网络模式也可以对目标服务进行多步通信。一个多步网络模板的示例是CVE-2015-3306exploit-tools。
...
DNS查询中要解析的域名或子域名。通常情况下，会使用变量&lbrace;&lbrace;FQDN&rbrace;&rbrace;来指定，例如：
```bash
...
脚本挂钩了一些内置的sink，如innerHTML属性、document.write()和eval（），使用navigate action加载页面，然后使用另一个script action将有效载荷"&lbrace;&lbrace;randstr_1&rbrace;&rbrace;""&lt;&gt;"注入到目标页面的window.name属性中。
...
HTTP流水线是HTTP协议（从1.1版本开始）的一项功能，它允许在同一个TCP连接上发送多个请求，而无需等待每个对应的响应。这与连接重用不同，在连接重用中，即使使用相同的TCP连接进行多个请求客户端也会在发出下一个请求之前等待服务器的响应。流水线在从web-servers获取资源时优化了网络延迟和带宽。
